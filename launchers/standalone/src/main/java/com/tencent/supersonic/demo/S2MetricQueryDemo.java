...
@Slf4j
@Order(4)
public class S2MetricQueryDemo extends S2BaseDemo {

    private static final String PLUGIN_NAME = "metric_query插件";
    private static final String PLUGIN_SERVICE_NAME = "metric_query";
    private static final String AGENT_NAME = "metric_query智能体";

    @Override
    public void doRun() {
        try {
            ChatPlugin plugin = addMetricQueryPlugin();
            Long pluginId = plugin != null ? plugin.getId() : null;
            addMetricQueryAgent(pluginId);
        } catch (Exception e) {
            log.error("Failed to add metric query demo data", e);
        }
    }
    ...
    private ChatPlugin addMetricQueryPlugin() {
    ...
    private void addMetricQueryAgent(Long pluginId) {
        if (pluginId == null) {
        ...
        agent.setDescription(String.format("通过调用 %s 插件完成指标查询相关对话", PLUGIN_NAME));
        agent.setStatus(1);
