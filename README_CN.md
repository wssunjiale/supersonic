...
## Superset 6.0.0 图表 form_data 模板抽取（用于 API 创建图表）

目标：对每个 `viz_type` 给出两类模板：
- `defaultFormData`：Explore UI 实际生成的默认 `form_data`（来源：浏览器/Redux store）
- `minimalFormData`：基于 UI 必填信号（requiredUnionKeys）裁剪得到的“最小可用”模板（缺失字段用 `null` 占位）

### 1) 从浏览器 UI 抽取 required + default form_data

需要一个可访问的 Superset 实例（例如内网地址），并通过环境变量提供登录信息：
- `SUPERSET_BASE_URL`（例如 `http://10.0.12.244:8088`）
- `SUPERSET_USERNAME`
- `SUPERSET_PASSWORD`

运行（示例仅抽取 BigNumber）：
```powershell
cd D:\vsx\metrics\supersonic
$env:SUPERSET_BASE_URL="http://10.0.12.244:8088"
$env:SUPERSET_USERNAME="admin"
$env:SUPERSET_PASSWORD="admin123456"
python scripts\extract_required_controls_from_superset_ui_modal_v2.py --only big_number
```

输出：
- `D:\vsx\metrics\supersonic\output\playwright\ui.required-controls.6.0.0.json`

### （可选）刷新 viztype 列表（以 /chart/add 的 All charts 为准）

如果怀疑 `viztype-6.0.0.json` 不全，可以先用 /chart/add 生成报告，再用报告更新本地资产：
...
$env:SUPERSET_USERNAME="admin"
$env:SUPERSET_PASSWORD="admin123456"
python scripts\extract_chart_add_viztypes_and_update_schemas.py --base-url $env:SUPERSET_BASE_URL
# 使用生成的 report 更新 viztype-6.0.0.json（会自动备份）
python scripts\update_viztype_6_0_0_from_chart_add_reports.py --reports output\playwright\chart-add.extract-and-update.*.report.json
```

注意：`extract_chart_add_viztypes_and_update_schemas.py` 会更新 `form-data-schema/` 下的 `图表创建字段`（会生成 .bak 备份）。

### 2) 生成可提交的模板资产（default/minimal）

```powershell
cd D:\vsx\metrics\supersonic
python scripts\build_form_data_templates_from_ui_extract.py `
  --ui-extract output\playwright\ui.required-controls.6.0.0.json
```

输出目录：
- `D:\vsx\metrics\supersonic\form-data-templates\default-form-data.6.0.0.json`
- `D:\vsx\metrics\supersonic\form-data-templates\minimal-form-data.6.0.0.json`
- `D:\vsx\metrics\supersonic\form-data-templates\required-controls.6.0.0.json`

### 3) PoC：用 API 创建图表并对比服务端返回的 params(form_data)
...
$env:SUPERSET_USERNAME="admin"
$env:SUPERSET_PASSWORD="admin123456"
python scripts\poc_create_chart_and_compare_form_data.py --viz-type big_number
```

输出：
- `D:\vsx\metrics\supersonic\output\poc\chart.create_and_compare.6.0.0.json`
